...
> 一站式行情总览面板，支持现货/币币/合约分类筛选，内置热门、DeFi、公链、Meme、AI 等板块标签，实时展示币种价格、24h 涨跌幅、成交额和价格走势，支持自选收藏和搜索。

![首页看板](docs/screenshots/01-home.png)

### 行情监控

> 专业级 K 线图表和订单簿深度图，支持 BTC/USDT 等主流交易对，提供 1m/5m/15m/1h/4h/1d 多个时间周期切换，实时 WebSocket 推送最新价格和涨跌变动。

![行情监控](docs/screenshots/02-market.png)

### 交易下单

> 支持现货和合约两种交易模式，提供市价/限价下单方式，买入卖出一键操作。右侧集成资产总览、当前挂单和历史订单查询，交易风险提示清晰可见。

![交易下单](docs/screenshots/03-trading.png)

### 策略中心

> 策略研发与管理一体化平台，内置多时间框架、资金费率套利、均值回归、动量突破等十余种专业量化策略模板。支持策略搜索、风险等级标注、适用币种和周期标签，可直接查看/编辑策略代码和参数。

![策略中心](docs/screenshots/04-strategy.png)

### 策略回测

> 基于 v2 回测引擎，支持选择策略、交易对、K 线周期、起止日期、初始资金和手续费率等参数配置。回测完成后展示夏普比率、最大回撤、收益曲线等专业绩效分析指标。

![策略回测](docs/screenshots/05-backtest.png)

### 模拟盘 / 实盘交易

> 支持模拟盘和实盘两种模式，采用向导式流程：选择策略 → 配置参数 → 飞行检查 → 运行监控。策略卡片展示回测绩效、夏普比率、最大回撤等关键指标，帮助快速决策。

![模拟盘/实盘](docs/screenshots/06-live.png)

### 监控中心

> 实时监控面板，展示多空比、持仓量、运行中策略数和活跃告警数。支持自定义告警规则（价格高于/低于、价格变动、费率高于/低于），集成 Telegram 推送通知。

![监控中心](docs/screenshots/07-monitor.png)

### 数据管理中心

> K 线历史数据的统一管理平台，支持全量同步、增量更新和自定义同步。按交易对和时间周期（1分钟 ~ 日线）展示数据量、覆盖天数和新鲜度，一键管理近 700 万条历史 K 线记录。

![数据管理中心](docs/screenshots/08-data.png)
